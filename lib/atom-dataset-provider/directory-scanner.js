...
var makePathVars = function(pathstr, home) {
    ...
    console.log("makepathvars");
    console.log(home);
    relpathstr = pathstr.substring(home.length-1)
    console.log(relpathstr);
    re = /^\/([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?([^/]+\/)?/
    relr = re.exec(relpathstr);
    fullr = re.exec(pathstr);
    console.log("relr:" + JSON.stringify(relr));
    p = {};
    for (i = 1; fullr && fullr[i]; i++)
        p["path" + i] = fullr[i].substring(0, fullr[i].length-1); // remove trailing / 
...
var combineGroupedDatasets = function(datasets, options, directory) {
  ...
  _.defaults(p, {
    ...
    'entrytitle': getEntryTitle(datasets, options),
