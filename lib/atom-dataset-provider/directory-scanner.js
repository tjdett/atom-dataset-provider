...
var scan = function(directory, options, callback) {
  ...
  // if passed a filename to "--directory", treat its contents as a list of files.
  ...
  if (options.fileList != '') {
    ...
    Fs.find(directory,
      {
      /* We want this to be much faster.
      1. sudo apt-get install -y inotify-tools
      #TODO: fix datapuller's RHEL issue (not registered? broken registration?)
      2. Add this background task: 
      inotifywait -e modify -mrq --format "%w%f" /mnt/np_staging >> dirchanges.log
      (It will have two entries for each directory per file change, one per file creation. Add %f for filenames.

      3. Add option to support reading directory names from file rather than scanning directory
      4. If option provided:
        a. Call a different function instead of Fs.find(), which:
        b. renames the file (to avoid race condition):
          fs.renameSync(filename, filename + '.wip');
          filename = filename + '.wip';
        b. reads that file into memory, one entry per line, removing duplicates: 
          filenames = fs.readFileSync(filename).toString().split("\n");
          filenames = _.uniq(filenames);        
        d. Must populate the results[] array, including calling fs.stat() on each file.
           _.each(filenames, function (f) { results[f] = { 'file': f, 'stat': fs.stat(f); })
      */
    ...
    function(err) { process_results(results); });
  };
}
